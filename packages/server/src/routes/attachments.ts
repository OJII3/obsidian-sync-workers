...
export function attachmentChangesHandler(env: Env) {
	return async (context: any) => {
		...
		const { changes: changesList, lastSeq } = await db.getAttachmentChanges(vaultId, since, limit);
...
export function attachmentMetaHandler(env: Env) {
	return async (context: any) => {
...
export function attachmentContentHandler(env: Env) {
	return async (context: any) => {
		const { params, query, set } = context;
		const id = decodeURIComponent(params.id);
		const vaultId = query.vault_id || "default";
...
export function uploadAttachmentHandler(env: Env) {
	return async (context: any) => {
		const { params, query, request, set } = context;
